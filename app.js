...
function init(){
  ...
  // Register service worker (optional PWA cache)
...
function renderCalendar(){
  ...
  // We start weeks on Monday: compute grid start = Monday before/at first day
...
function syncYearFilterOptions(){
  ...
  if (sorted.includes(parseInt(current,10))) {
    els.filterYear.value = current;
  } else {
    els.filterYear.value = `${state.viewDate.getFullYear()}`;
  }
}
...
function saveFromModal(){
  ...
  if (id) {
    state.entries = state.entries.map(x => x.id === id ? { ...x, ...payload } : x);
  } else {
...
function exportICS(){
  // Very simple all-day VEVENT export; end date is exclusive in iCal.
  const year = parseInt(els.filterYear.value || `${state.viewDate.getFullYear()}`, 10);
  const typeFilter = els.filterType.value || "all";
